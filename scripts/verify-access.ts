...
for (const env of environments) {
  ...
  let toolsJson = false;
  try { JSON.parse(tools.out); toolsJson = true; } catch { /* reported below */ }
  passed = check('public tools manifest', tools.code === 0 && toolsJson) && passed;
  ...
  if (requireProducer) {
    const producer = /- Roles:.*\b(producer|admin)\b/i.test(auth.out);
    passed = check('producer-capable role', producer) && passed;
  }
