...
function kli(args: string[], cwd = ROOT, timeout = 120_000) {
  const r = spawnSync('bun', ['src/cli.ts', '--env', fixture.env, ...args], { cwd, timeout, stdio: 'pipe' });
...
async function main() {
  ...
  const stages: Stage[] = [
    { name: 'health', expectation: 'kli health exits 0 and reports healthy', async run() {
      ...
      return { pass: ok, evidence: [log('health', 'exit 0 + healthy', `exit ${r.code}; ${compact(r.out + r.err)}`, ok)] };
    } },
    { name: 'auth', expectation: 'valid PKCE authentication is available (refresh once if expired)', async run() {
      ...
      const evidence = [log('auth', 'stored valid authentication', status.hasTokens ? (status.expired ? 'expired token' : 'valid token') : 'no token', status.hasTokens && !status.expired)];
      if (status.hasTokens && status.expired) {
        ...
        evidence.push(log('auth', 'refresh succeeds', `exit ${refresh.code}`, ok));
      }
    ...
    { name: 'discover', expectation: 'init with local manifest writes AGENTS.md with >=14 units and >0 tools', async run() {
      ...
      return { pass: ok, evidence: [log('discover', 'exit 0, AGENTS.md, >=14 units, >0 tools', `exit ${r.code}; AGENTS=${!!content}; units=${units}; tools=${tools}`, ok)] };
    } },
    { name: 'media-in', expectation: 'both calibrated media files are present and upload-analyze returns file IDs', async run() {
      ...
      for (const track of fixture.tracks) {
        ...
        evidence.push(log('media-in', `${track.key} uploaded with fileId`, `exit ${r.code}; fileId=${id || 'missing'}`, ok));
    ...
    { name: 'analysis-qc', expectation: 'analysis succeeds with calibrated BPM and beat grid; start time is advisory when configured', async run() {
      ...
      const evidence: string[] = []; const api = resolveApiUrl(fixture.env);
      for (const track of fixture.tracks) {
        const id = context.fileIds[track.key]; let data: any; let status = 'UNKNOWN';
        for (let attempt = 0; attempt < 60; attempt++) {
          ...
          if (a.ok) { data = await a.json(); const jobId = data?.analysisJob?.jobId; if (jobId) { const j = await fetch(`${api}/api/jobs/${encodeURIComponent(jobId)}`, { headers: { Authorization: `Bearer ${bearer}`, Accept: 'application/json' } }); if (j.ok) { const job = await j.json(); status = job.status || job.job?.status || 'UNKNOWN'; if (status === 'SUCCEEDED') { const fresh = await fetch(`${api}/api/multimedia/${encodeURIComponent(id)}/analysis`, { headers: { Authorization: `Bearer ${bearer}`, Accept: 'application/json' } }); if (fresh.ok) data = await fresh.json(); break; } if (['FAILED', 'CANCELLED', 'TIMED_OUT'].includes(status)) break; } } }
          await Bun.sleep(5000);
        }
        const dna = data?.musicDNA ?? data?.analysis?.musicDNA ?? data?.analysis ?? data?.result?.musicDNA;
        const bpm = Number(dna?.bpm ?? data?.bpm); const start = Number(dna?.start_time ?? data?.start_time); const grid = dna?.beat_grid ?? dna?.beatGrid ?? data?.beat_grid ?? data?.beatGrid;
        const startTimeMatches = Number.isFinite(start) && Math.abs(start - track.expectedStartTimeMs) <= track.startTimeToleranceMs;
        const ok = status === 'SUCCEEDED' && Number.isFinite(bpm) && matchBpm(bpm, track.expectedBpm, track.bpmTolerance, track.acceptBpmOctaves) && (track.startTimeAdvisory || startTimeMatches) && Array.isArray(grid) && grid.length > 0;
        evidence.push(log('analysis-qc', `${track.key}: job SUCCEEDED, BPM/grid calibrated`, `status=${status}; bpm=${bpm}; start=${start}; grid=${Array.isArray(grid) ? grid.length : 0}`, ok));
        if (!startTimeMatches && track.startTimeAdvisory) evidence.push(warn('analysis-qc', `${track.key}: start_time within ${track.startTimeToleranceMs}ms of ${track.expectedStartTimeMs} (advisory)`, `start=${start}`));
        if (!ok) return { pass: false, evidence, error: `Analysis QC failed for ${track.key}; response did not contain the required calibrated MusicDNA fields.` };
        context.bpms[track.key] = bpm;
      } return { pass: true, evidence };
    } },
    { name: 'compose', expectation: 'a beats-only komposition is loaded and appears in kompositions', async run() {
      ...
      context.kompositionId = id; return { pass: ok, evidence: [log('compose', 'load-file and kompositions listing identify the new ID', `load=${loaded.code}; id=${id || 'missing'}; list=${listed.code}`, ok)] };
    } },
    { name: 'build', expectation: 'render-qc succeeds and a production stream URL resolves', async run() {
      ...
      return { pass: ok, evidence: [log('build', 'render succeeds and production stream resolves', `render=${rendered.code}; stream=${url ? 'present' : 'missing'}`, ok)] };
    } },
  ...
  try { for (const stage of stages) { const result = await stage.run(); report.stages.push({ name: stage.name, expectation: stage.expectation, ...result }); if (!result.pass) { if (result.error) console.error(`[${stage.name}] ${result.error}`); break; } } }
